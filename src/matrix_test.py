...
class TestCheckNeighbors(unittest.TestCase):
    """Execute a test covering the GOL rules."""
...
def is_cell_living(matrix, x, y):
    """Return if a cell is living (true) os not (False)."""
...
def check_neighbors(matrix, x, y):
    """Check a cell neighbors according to the existent rules of GoL."""
    ...
    if (not is_cell_living(matrix, x, y) and
       living_cells_count == BIG_POPULATION):
        """A living cell with a big population neighbors."""
        return LIVE
...
def lives_search(matrix):
    """Execute a search through the 2d matrix."""
